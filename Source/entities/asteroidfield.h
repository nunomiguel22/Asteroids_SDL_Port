...
typedef struct {
  ...
  unsigned int
      death_timer; /**< @brief Asteroid death timer for death animation */
  int death_frame;
...
/**
 * @brief Initiates all large asteroids for the round with random movement, the
 * number of asteroids increases with the number of rounds
 *
...
/**
 * @brief Updates the position of all active asteroids, when crossing screen
 * bounds warps asteroid to the other edge of the screen
 *
 * @param asteroid_field Array of asteroids
 */
bool ast_update(asteroid asteroid_field[]);
/**
 * @brief Fragments large asteroids into two smaller asteroids with random
 * movement
 *
 * @param asteroid_field Array of asteroids

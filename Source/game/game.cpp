      ...
      switch (game->SDLevent.type) {
        ...
        case SDL_KEYDOWN: {
          switch (game->SDLevent.key.keysym.sym) {
            case SDLK_w:
              game->player1.s_event = MAIN_THRUSTER;
              break;
            case SDLK_a:
              game->player1.s_event = PORT_THRUSTER;
              break;
            case SDLK_d:
              game->player1.s_event = STARBOARD_THRUSTER;
              break;
            case SDLK_s:
              game->player1.s_event = REVERSE_THRUSTER;
              break;
            case SDLK_ESCAPE:
              game->player1.s_event = K_ESC;
              break;
            case SDLK_SPACE:
              game->player1.s_event = QUIT;
              break;
            case SDLK_F3: {
...
void game_state_machine(game_data* game) {
  switch (game->state) {
    case MENU: {
      main_menu_handler(game);
      break;
    }

    case OPTIONSMENU: {
      options_menu_handler(game);
      break;
    }

    case SP_START_SEQUENCE: {
      singleplayer_start_sequence(game);
      break;
    }
    ...
    case SINGLEPLAYER: {
      ...
      if (game->player1.status & BIT(4)) {
        if (game->event == TIMER) round_delay++;
    ...
    case SP_LOSS: {
      singleplayer_game_over(game);
      break;
    }

    case SP_GAMEPAUSED: {
      singleplayer_pause(game);
      break;
    }
    ...
    case MP_START_SEQUENCE: {
      multiplayer_start_sequence(game);
      break;
    }
    ...
    case MP_ROUND: {
      ...
      if (!delay && game->player2.hp <= 0) {
        ...
        game->timers.player2_death_timer =
            (unsigned int)(ALIEN_DEATH_DURATION * 60);
      ...
      if (!delay && game->player1.hp <= 0) {
        game->threads.push(play_sound, std::ref(*game->sound.ship_expl));
        game->timers.player1_death_timer =
            (unsigned int)(ALIEN_DEATH_DURATION * 60);
        delay = game->timers.timerTick + 60;

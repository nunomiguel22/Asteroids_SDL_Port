...
int game_data_init(game_data *game) {
	...
	if (game->ttf_fonts.lucida_console_med.load(font_path, CON_FONT_SIZE)) {
		game->console.log_push("Couldn't load \"" + font_path + "\", error: " + SDL_GetError());
		return 1;
	}
	else game->console.log_push("Loaded \"" + font_path + "\"");
...
void physics_update(int id, game_data &game) {
	...
	else game.player1.status &= ~BIT(3);
...
void exec_console_cmd(std::string cmd, game_data* game) {
	...
	case con_not_defined: { game->console.write("No such command", C_ERROR); return; }
		...
		case con_init_server: {
			...
			if (game->connection.port_open(port)) {
				...
				if (game->connection.fecth_public_ip()) 
					game->console.write("   Public IP: " + game->connection.get_public_ip() , C_NORMAL);
				
				else game->console.write("   Public IP: Not connected to the internet", C_WARNING);
			}
			else game->console.write("Failed to open port...", C_ERROR);
		...
		case con_init_client: {
			if (game->connection.port_is_open()) {
				...
				if (game->connection.server_is_open()) {
					game->console.write("Server mode detected, closing server...", C_WARNING);
			...
			if (game->connection.port_open(DEFAULT_CLIENT_PORT)) {
				if (!game->connection.fecth_public_ip())
					game->console.write("No internet connection, local network only", C_ERROR);

			}
			else game->console.write("Failed to open port 1337...", C_ERROR);
		...
		case con_host_game: {
			...
			else game->console.write("You need to start a server to host a game, use init_server 'port'", C_ERROR);
		...
		case con_connect: {
			...
			if (game->connection.server_is_open() || !game->connection.port_is_open()) {
				game->console.write("To connect to a game use client mode by entering 'init_client' in the console", C_ERROR);
				break;
			}
		...
		case con_disconnect: {
			...
			else game->console.write("Not currently in a online game", C_ERROR);
		...
		case con_help: {
			...
			game->console.write("init_server portnumber - Initiates server listening to given port number", C_WHITE);
			game->console.write("init_client            - Initiates network in client mode", C_WHITE);
			game->console.write("host_game              - Initiates network game if server mode has been started", C_WHITE);
			game->console.write("connect ip:port        - Connects to a network game hosted by a server with given ip and port number", C_WHITE);
			game->console.write("disconnect             - Disconnects from a network game", C_WHITE);

...
uint32_t *init_sdl(game_settings *settings, console *cons) {
  ...
  if (settings->vsync) rendererFlags |= SDL_RENDERER_PRESENTVSYNC;

  /* Window */
  window = SDL_CreateWindow("Asteroids", SDL_WINDOWPOS_UNDEFINED,
                            SDL_WINDOWPOS_UNDEFINED, settings->hresolution,
                            settings->vresolution, windowFlags);
  if (window == NULL) {
    std::string error = SDL_GetError();
    cons->log_push("Failed to create window, error: " + error);
    return NULL;
  } else
    cons->log_push("Created window");
  ...
  if (pixelbuffer == NULL) {
    ...
    cons->log_push("Created pixel buffer");
  ...
  if (Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 2, 4096) == -1) {
    ...
    cons->log_push("Audio API initialized");
  ...
  if (TTF_Init() < 0) {
    ...
    cons->log_push("TTF font API initialized");
  ...
  if (SDLNet_Init() < 0) {
    ...
    cons->log_push("Network API initialized");
...
mpoint2d vector_translate_gfx(mpoint2d *cartesian, unsigned int width,
                              unsigned int height) {
  mpoint2d gfx_point;
  mpoint2d origin;
...
int draw_pixel(int x, int y, uint32_t color) {
  if (x > hres - 1 || y > vres - 1 || x < 0 || y < 0) return 1;

  uint8_t alpha = color & 0x000000FF;
  if (alpha < 77) return 0;
  ...
  /* If the pixel is blank no need to blend, if foreground color is black there
   * is no need to write anything to the buffer */
  if (!fg_color && !bg_color) return 0;
...
void render_laser(player *p, Bitmap *laser, weapon *l) {
  ...
  laser->draw_transform((int)ws_laser.x, (int)ws_laser.y, l->get_angle(),
                        BMP_CENTER);
}
...
void draw_alien(game_data *game) {
  ...
  if (game->alien.status & BIT(0)) {
    ...
    game->bmp.alien_ship.draw_transform((int)ws_pivot.x, (int)ws_pivot.y,
                                        vcannon.angle() - 90, BMP_CENTER);

    // Draw alien lasers
    for (int i = 0; i < AMMO; i++) {
      if (game->alien.lasers[i].is_active()) {
        render_laser(&game->alien, &game->bmp.laser_red,
                     &game->alien.lasers[i]);
      }
    }
  }
    ...
    if (expl_frame > 9) expl_frame = 0;

    game->bmp.ship_expl[expl_frame].draw_transform(
        (int)ws_pivot.x, (int)ws_pivot.y, 0, BMP_CENTER);

    /*
            NEEDS TO BE MOVED TO INCREMENT TIMERS LATER, ANIMATION ON REPEAT
       WITH HIGH FRAMES
            

    */
    if (game->timers.alien_death_timer % 3 == 0) ++expl_frame;
  }
}

void draw_ast(asteroid *ast, Bitmap *bmp) {
  mpoint2d ws_ast = vector_translate_gfx(&ast->position, hres, vres);
  bmp->draw_transform((int)ws_ast.x, (int)ws_ast.y, (float)ast->degrees,
                      BMP_CENTER);
}
...
void render_crosshair(Bitmap *delta, player *player1) {
  ...
  delta->draw_transform((int)ws_mouse.x, (int)ws_mouse.y, vmouse.angle() - 90,
                        BMP_CENTER);
}
...
void render_frame(game_data *game) {
  ...
  for (int i = 0; i < AMMO; ++i) {
    if (game->player1.lasers[i].is_active()) {
      render_laser(&game->player1, &game->bmp.laser_blue,
                   &game->player1.lasers[i]);
    }
  }
  ...
  for (int i = 0; i < MAX_ASTEROIDS; ++i) {
    mpoint2d ws_ast =
        vector_translate_gfx(&game->asteroid_field[i].position, hres, vres);
    ...
    if (game->asteroid_field[i].active) {
      if (game->asteroid_field[i].size == MEDIUM)
        ...
        draw_ast(&game->asteroid_field[i], &game->bmp.large_asteroid);
    ...
    // Destruction animation
      ...
      if (game->asteroid_field[i].death_timer > 0) {
        ...
        game->bmp.ast_dest[game->asteroid_field[i].death_frame].draw_transform(
            (int)ws_ast.x, (int)ws_ast.y,
            (float)game->asteroid_field[i].degrees, BMP_CENTER);
      }
    }
  }
  ...
  if (game->timers.hitreg_timer > 0)
    ...
    render_crosshair(&game->bmp.delta, &game->player1);
  ...
  if (game->settings.fps_counter) {
    ...
    game->ttf_fonts.lucida_console_med.render_number(
        (float)game->timers.frames_per_second, 35, 10, C_GREEN);
  }

  // Draw Score
  game->ttf_fonts.copperplategothicbold.render_number(
      (float)game->player1.score, 195, 710, C_WHITE);
  ...
  // Draw round
  game->ttf_fonts.copperplategothicbold.render_number(
      (float)game->player1.round - 4, 195, 686, C_WHITE);

  // Draw Teleport status
  if (game->player1.status & BIT(6))
    game->ttf_fonts.copperplategothicbold.render_string(195, 662, "ready",
                                                        C_GREEN);
  else
    game->ttf_fonts.copperplategothicbold.render_string(195, 662, "down",
                                                        C_RED);

  draw_alien(game);

  if (game->console.is_open()) render_console(game);
}
...
void render_mp_frame(game_data *game) {
  ...
  if (game->player1.status & BIT(0)) {
    ...
    if (game->timers.player1_death_timer % 3 == 0) ++expl_frame;
  }
  ...
  if (game->player2.status & BIT(0)) {
    ...
    if (game->timers.player2_death_timer % 3 == 0) ++expl_frame;
  }
  ...
  if (game->player2.status & BIT(0)) {
    for (int i = 0; i < AMMO; ++i) {
      if (game->player2.lasers[i].is_active()) {
        render_laser(&game->player2, &game->bmp.laser_red,
                     &game->player2.lasers[i]);
      }
    }
  }
  ...
  if (game->state == MP_ROUND) {
    ...
    game->ttf_fonts.copperplategothicbold_massive.render_string(
        140, 20, game->player1.name, C_WHITE);
    game->bmp.mp_scoreplate.draw(350, 20);
    game->ttf_fonts.copperplategothicbold_massive.render_number(
        game->player1.mp_round, 355, 20, C_WHITE);
    game->bmp.mp_hpbaroutline.draw(120, 80);
    render_mp_hp_bars(&game->player1, 125, 85);

    game->ttf_fonts.copperplategothicbold_massive.render_string(455, 20, "VS",
                                                                C_WHITE);

    game->bmp.mp_scoreplate.draw(578, 20);
    game->ttf_fonts.copperplategothicbold_massive.render_number(
        game->player2.mp_round, 583, 20, C_WHITE);
    game->bmp.mp_nameplate.draw(632, 20);
    game->ttf_fonts.copperplategothicbold_massive.render_string(
        642, 20, game->player2.name, C_WHITE);
  ...
  if (cnt_msg) {
    if (game->timers.timerTick < cnt_time)
      game->ttf_fonts.lucida_console_med.render_string(
          10, 50, game->player2.name + " joined ", C_WHITE);
    else
      cnt_msg = false;
  }
  ...
  if (game->state == MP_WARMUP) {
    if (cnt_time) {
      if (game->player1.status & BIT(1)) {
        game->ttf_fonts.copperplategothicbold.render_string(
            420, 150, "Press F3 to ", C_WHITE);
        game->ttf_fonts.copperplategothicbold.render_string(520, 150, "unready",
                                                            C_RED);
      } else {
        game->ttf_fonts.copperplategothicbold.render_string(
            420, 150, "Press F3 to ", C_WHITE);
        game->ttf_fonts.copperplategothicbold.render_string(
            520, 150, "ready up", C_GREEN);
      }
    } else
      game->ttf_fonts.copperplategothicbold.render_string(
          420, 150, "Waiting for enemy", C_WHITE);
  }

  if (game->console.is_open()) render_console(game);
}

void handle_frame_wrapper(int id, game_data &game) { handle_frame(&game); }

void render_seq_frame(game_data *game) {
  game->bmp.game_background.draw(0, 0);
  draw_ship(&game->bmp.pix_ship_blue, &game->player1);
  game->ttf_fonts.copperplategothicbold_massive.render_number(
      (float)game->timers.start_seq, 512, 304, C_WHITE);
}
...
void render_console(game_data *game) {
  ...
  game->ttf_fonts.lucida_console_med.render_string(
      5, VERT_BAR_Y_POS, game->console.input(), C_WHITE);

  std::queue<consolemessage> tempmessages =
      game->console.get_console_messages();

  int messagessize = tempmessages.size();

  for (int i = 0; i < messagessize; ++i) {
    game->ttf_fonts.lucida_console_med.render_string(
        15, 15 + (i * 14), tempmessages.front().message,
        tempmessages.front().messagecolor);
    tempmessages.pop();
  }
}
...
void handle_menu_frame(game_data *game, Bitmap *bckgrd) {
  ...
  switch (game->state) {
    case MENU: {
      for (int i = 0; i < MAX_ASTEROIDS; i++) {
        mpoint2d ws_ast = vector_translate_gfx(
            &game->menu_asteroid_field[i].position, 1024, 768);
        // Active asteroids
        if (game->menu_asteroid_field[i].active) {
          if (game->menu_asteroid_field[i].size == MEDIUM)
            ...
            draw_ast(&game->menu_asteroid_field[i], &game->bmp.large_asteroid);
        }
      }
      ...
      if (game->SDLevent.motion.x > 395 && game->SDLevent.motion.x < 573) {
        if (game->SDLevent.motion.y > 200 && game->SDLevent.motion.y < 228) {
          ...
          playb_color = C_WHITE;
        } else if (game->SDLevent.motion.y > 240 &&
                   game->SDLevent.motion.y < 268) {
          hostb = &game->bmp.hl_button;
          hostb_color = C_WHITE;
        } else if (game->SDLevent.motion.y > 280 &&
                   game->SDLevent.motion.y < 308) {
          cntb = &game->bmp.hl_button;
          cntb_color = C_WHITE;
        } else if (game->SDLevent.motion.y > 320 &&
                   game->SDLevent.motion.y < 348) {
          optb = &game->bmp.hl_button;
          optb_color = C_WHITE;
        } else if (game->SDLevent.motion.y > 360 &&
                   game->SDLevent.motion.y < 388) {
          gitb = &game->bmp.hl_button;
          gitb_color = C_WHITE;
        } else if (game->SDLevent.motion.y > 400 &&
                   game->SDLevent.motion.y < 428) {
      ...
      playb->draw(395, 200);
      game->ttf_fonts.copperplategothicbold.render_string(460, 205, "Play",
                                                          playb_color);

      hostb->draw(395, 240);
      game->ttf_fonts.copperplategothicbold.render_string(440, 245, "Host game",
                                                          hostb_color);

      cntb->draw(395, 280);
      game->ttf_fonts.copperplategothicbold.render_string(
          413, 285, "Connect to game", cntb_color);

      optb->draw(395, 320);
      game->ttf_fonts.copperplategothicbold.render_string(445, 325, "Settings",
                                                          optb_color);

      gitb->draw(395, 360);
      game->ttf_fonts.copperplategothicbold.render_string(
          428, 365, "Github repo", gitb_color);

      quitb->draw(395, 400);
      game->ttf_fonts.copperplategothicbold.render_string(460, 405, "Exit",
                                                          quitb_color);

      for (int i = 0; i < 5; i++)
        game->ttf_fonts.copperplategothicbold.render_number(
            (float)game->highscores[i], 900, 60 + i * 40, C_WHITE);
    ...
    case OPTIONSMENU: {
      /* FPS Counter */
      if (game->settings.fps_counter) game->bmp.boxticked.draw(348, 270);
      /* Vsync */
      if (game->settings.vsync) game->bmp.boxticked.draw(151, 268);
      /* Resolution */
